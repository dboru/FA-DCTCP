...
#define LFLOW_PORT_LOWER_BOUND 8000
#define LFLOW_PORT_UPPER_BOUND 9000
/* alpha (dctcp_alpha) ranges from 0 to 1024, so (.6 * alpha) is calculated as ->  .6 * 1024 ~ 615 */
#define POINTSIX_TIMES_ALPHA 615
#endif
		...
		if(!sysctl_tcp_dctcp_enable) {
			...
			tp->snd_cwnd = min(tp->snd_cwnd,
					tcp_packets_in_flight(tp) + 1U);
                        ...
                        if ((ntohs(inet->inet_dport) >= LFLOW_PORT_LOWER_BOUND) && 
                            (ntohs(inet->inet_dport) <= LFLOW_PORT_UPPER_BOUND))
			{  
			    cwnd_new = max (tp->snd_cwnd - ((tp->snd_cwnd * tp->dctcp_alpha)>>11) , 2U);
                        }
                        else
                        {   
                            /* if alpha > .6 reduce the cwnd more rapidly;  
                               cwnd = cwnd * (1 - (alpha * alpha) / 2 )  */  
                            /* since as per original implementation alpha ranges from 0 to 1024;
                               (.6 * alpha) is calculated as ->  .6 * 1024 ~ 615 */
                            if (tp->dctcp_alpha > POINTSIX_TIMES_ALPHA)
                            {
                                /* Explanation for use of 21 
                                   2^10 = 1024,  is to normalize for alpha which is in the range 0 - 1024 
                                   2^11 (normalizes and brings in the 1/2 term in to the picture) = 2048
                                    -> 10 + 11 = 21   */
                                cwnd_new = max (tp->snd_cwnd - 
                                            ((tp->snd_cwnd * tp->dctcp_alpha * tp->dctcp_alpha)>>21) , 2U);
                            }
                            else /* cwnd = cwnd * (1 - (alpha * alpha * alpha) / 2 )  */
                            {
                                /* Explanation for use of 31 
                                   2^10 = 1024,  is to normalize for alpha which is in the range 0 - 1024 
                                   2^10 = 1024,  is to normalize for alpha which is in the range 0 - 1024 
                                   2^11 (normalizes and brings in the 1/2 term in to the picture) = 2048
                                    -> 10 + 10 + 11 = 31   */
                                cwnd_new = max (tp->snd_cwnd - 
                                            ((tp->snd_cwnd * tp->dctcp_alpha * tp->dctcp_alpha  
                                                           * tp->dctcp_alpha)>>31) , 2U);
                            }  
                        }
			if(set_ssthresh) {
